...
<div class="container mx-auto px-6 py-8">
    <h3 class="text-gray-700 text-3xl font-medium mb-6">Tambah Batas Desa</h3>

    <div class="bg-white shadow-lg rounded-xl overflow-hidden p-6">
        <form action="{{ route('admin.administrative-boundaries.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-4">
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('name') border-red-500 @enderror" id="name" type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Desa Tegalsambi">
                @error('name')
            ...
            <div class="mb-6">
                ...
                <p class="text-gray-500 text-xs italic mt-1">Upload file GeoJSON yang berisi geometri polygon wilayah.</p>
                @error('geojson_file')
                    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                @enderror
            </div>
